...
# dataset.append(numpy.loadtxt("ecg/abnormal/801_*.0", delimiter="   ", usecols=[2]))
# dataset.append(numpy.loadtxt("ecg/abnormal/801_*.1", delimiter="   ", usecols=[2]))


# for data in dataset:
#     pass

# [(file) for file in glob.glob('/ecg/abnormal/*.0')]
# dataset = numpy.zeros([75,2])


# for file in glob.glob("/ecg/abnormal/*.0"):
#    dataset.append(np.loadtxt(file))
...
for i in range(100000):
    ...
    loss_train = loss_fn(y_pred_train,Y_train[:621])
