...
if local:
    dataset = tf.jsonfile_reader('training-data/mydata*.json')
else:
...
x_train_temp, unique_words = tf.natural_lang_process_all_questions(x_train1)
print("Done!")
...
for each in x_train_temp:
    x_train.append(tf.make_text_into_numbers(each, max_words, unique_words))
...
x_test_temp, test_nique_words = tf.natural_lang_process_all_questions(x_test1)
print("Done!")
...
for each in x_test_temp:
    x_test.append(tf.make_text_into_numbers(each, max_words, test_nique_words))
...
nene =  tf.training_from_file(use_pretrained_model=False, n_steps=n_steps, x_train=x_train, y_train=y_train, file_name=file_name,
                      len_unique_words=len(unique_words), questions=questions, max_words=max_words,  y_test=y_test,  x_test=x_test, y_train_org=y_train_org, y_test_org=y_test_org)
''' --------------------- TRAIN ---------------------'''
