...
def training_from_file(use_model, n_steps, x_train, y_train, file_name, unique_words, questions, max_words, y_test,
                       x_test, y_train_org, y_test_org):
    train_acc = []
    validate_acc = []
            ...
            if (i % 250) == 0:
                ...
                for q in x_train:
                    ...
                    if category == y_train_org[j]:
                        count += 1
                    j += 1
                train_acc.append(count / len(x_train))
                k = 0
                test_count = 0
                for t in x_test:
                    ...
                    if test_category == y_test_org[k]:
                        test_count += 1
                    k += 1
                validate_acc.append(test_count / len(x_test))
                print("loss:", loss_train.cpu().detach().numpy(), "- step:", i, " Training Accuracy: ",
                      count / len(questions), " Testing Accuracy: ", test_count / len(x_test))
