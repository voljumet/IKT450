...
def load_data(dataset, local):
	...
	for each in dataset:
		if local:
			...
			result = cf(' '.join(each['question']['tokens']))
			label.append(result['labels'][0])
...
def training_from_file(use_model, n_steps, x_train, y_train, file_name, unique_words, questions, max_words, y_test ,x_test,x_train1, x_test1, y_train_org, y_test_org):
			...
			if (i % 1) == 0:
				...
				for q in x_train1:
					...
					if category == y_train_org[j]:
						count += 1
					j += 1
				train_acc.append(count / len(x_train1))
				k = 0
				test_count = 0
				for t in x_test1:
					...
					if test_category == y_test_org[k]:
						test_count += 1
					k += 1
				validate_acc.append(test_count / len(x_test1))
				print("loss:", loss_train.cpu().detach().numpy(), "- step:", i, " Training Accuracy: ", count / len(x_train1), " Testing Accuracy: ", test_count / len(x_test1))
