...
def get_prediction(img_path, threshold=0.5, url=False):
  ...
  for count, list1 in enumerate(pred_boxes):
      for count2, list2 in enumerate(list1):
          for count3, i in enumerate(list2):
              pred_boxes[count][count2] = tuple(map(int, list2))
...
img, pred_classes, masks = instance_segmentation('/Users/peshangalo/Documents/Master/First_Year/DNN/CNN_2/10best-cars-group-cropped-1542126037.jpg', rect_th=5, text_th=2)
plt.imshow(img)
plt.show()